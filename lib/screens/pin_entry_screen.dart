...
class _PinEntryScreenState extends State<PinEntryScreen> {
  ...
  Future<void> saveOrder(Map order) async {
    final prefs = await SharedPreferences.getInstance();

    await prefs.setBool('active_order_exists', true);

    await prefs.setString('active_order_service', order['service'] ?? '');
    await prefs.setInt('active_order_qty', order['qty'] ?? 1);
    await prefs.setString('active_order_pickup', order['pickupTime'] ?? '');
    await prefs.setString('active_order_delivery', order['deliveryTime'] ?? '');
  }

...
class OrderDetailScreen extends StatelessWidget {
  ...
  Future<void> _saveOrderToSharedPreferences({
    ...
    final List<String> existingOrders =
        prefs.getStringList('active_orders') ?? [];
    existingOrders.add(orderString);
    await prefs.setStringList('active_orders', existingOrders);

    debugPrint('Pesanan baru disimpan: $orderId dengan total: $totalPrice');
  ...
  Widget build(BuildContext context) {
    ...
    final bool isFromProcessOrder = args['fromProcessOrder'] == true;

    final String orderIdTemp = args['orderId']?.toString() ?? '';
    final bool isDummyOrder = orderIdTemp == '100001';

    final int activeStepIndex = isDummyOrder ? 2 : 0;
    ...
    final String orderId = (isFromActiveOrder || isFromProcessOrder)
    ...
    if (orderItems.isNotEmpty) {
      ...
      totalProductPrice = orderItems.fold(0, (sum, item) {
        final qty = _toInt(item['qty']);
        final price = _toInt(item['price']);
        return sum + (qty * price);
      });
      ...
      totalProductPrice = _toInt(
        args['serviceFee'] != null ? args['serviceFee'] : 0,
      );
    }
    ...
    final String pickupAddress =
        args['address']?.toString() ?? 'Alamat belum diisi';
    final String deliveryAddress =
        args['deliveryAddress']?.toString() ?? pickupAddress;

    final int deliveryFee = _toInt(
      args['deliveryFee'] != null ? args['deliveryFee'] : 5000,
    );

    final int grandTotal;
    if (isFromActiveOrder || isFromProcessOrder) {
      final String savedTotalPrice = args['totalPrice']?.toString() ?? 'Rp 0';
      grandTotal = _toInt(savedTotalPrice);
    } else {
    ...
    final String serviceSummary = orderItems.isEmpty
        ...
        : orderItems
              .map(
                (item) => '${item['title'] ?? item['name']} (${item['qty']})',
              )
              .join(' + ');
    ...
    final String statusText = isDummyOrder ? 'Dicuci' : 'Diproses';
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      _box(
                        child: Column(
                          ...
                          children: [
                            ...
                            OrderStepProgressBar(activeIndex: activeStepIndex),
                            const SizedBox(height: 16),
                            ClipRRect(
                              borderRadius: BorderRadius.circular(16),
                              child: AspectRatio(
                      ...
                      if (orderItems.isNotEmpty)
                        _box(child: _buildOrderItems(orderItems)),
                        ...
                        _box(
                          child: Column(
                            ...
                            children: [
                              ...
                              _row(
                                'Subtotal Pesanan',
                                _formatRupiah(totalProductPrice),
                              ),
                            ],
                          ),
                        ),

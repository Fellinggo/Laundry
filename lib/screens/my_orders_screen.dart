...
class _MyOrdersScreenState extends State<MyOrdersScreen> {
  ...
  Future<void> _cleanInvalidOrders() async {
    ...
    if (cleaned) {
      print('Data invalid telah dibersihkan');
    }
  }
  ...
  Future<void> _loadOrders() async {
    ...
    if (isSignIn) {
      ...
      if (!hasDummy) {
        ...
        print('Dummy data injected for Sign In user');
      }
    }
    ...
    final List<String> completedRaw =
        prefs.getStringList('completed_orders') ?? [];
  ...
  String _getServiceTitle(Map<String, dynamic> order) {
    ...
    if (orderItems != null && orderItems.isNotEmpty) {
      final titles = orderItems
          .map((item) => item['title'] ?? item['name'] ?? 'Layanan')
          .toList();
      return titles.join(', ');
    }
  ...
  int _getTotalPrice(Map<String, dynamic> order) {
    ...
    final number = _parseRupiahToInt(totalPriceStr);
    return number;
  }
  ...
  Widget _buildOrderList(List<Map<String, dynamic>> orders, String status) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final int totalPrice = _getTotalPrice(order);

        final bool isDummyOrder = orderId == '100001';

        final int shippingFeeForDisplay = isDummyOrder ? 5000 : 0;
        final int grandTotal = totalPrice + shippingFeeForDisplay;

        final int deliveryFee = _safeToInt(order['deliveryFee'] ?? '5000');

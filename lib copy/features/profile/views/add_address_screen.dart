...
class _AddAddressScreenState extends State<AddAddressScreen> {
  ...
  void dispose() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        context.read<AddAddressController>().resetState();
      }
    });
    super.dispose();
  }
  ...
  void _saveAddress() {
    final provider = context.read<AddAddressController>();

    // Pemicu validasi di controller
    if (!provider.validate(addressController.text)) return;

    final addressData = provider.createAddress(addressController.text);
  ...
  Widget build(BuildContext context) {
    ...
    final addrProvider = context.watch<AddAddressController>();
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: addrProvider.titleError != null
                      ? Colors.red
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  ...
                  isExpanded: true,
                  items: addrProvider.titleOptions.map((title) {
                  ...
                  onChanged: (String? newValue) {
                    addrProvider.selectTitle(newValue);
                  },
                ),
              ),
            ),

            if (addrProvider.titleError != null)
              Padding(

...
import '../../shared/widgets/terms_bullet_widget.dart';
...
class TermsConditionsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<TermsController>(
...
class _TermsConditionsContent extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final controller = context.read<TermsController>();
...
class _TermsConditionsBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isLoading = context.select<TermsController, bool>((c) => c.isLoading);
    final errorMessage = context.select<TermsController, String?>(
      (c) => c.errorMessage,
    );
    final sections = context.select<TermsController, List<TermsSection>>(
      (c) => c.termsModel.sections,
    );
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: sections.map((section) {
              return _TermsSectionItem(section: section);
            }).toList(),
          ),
        ),
      ),
...
class _TermsSectionItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final int sectionIndex = section.title == 'Syarat Layanan'
        ? 0
        : (section.title == 'Kerusakan & Kehilangan' ? 1 : 2);
    final bool isLastSection = sectionIndex == 2;
    ...
    return Column(
      ...
      children: [
        ...
        ...section.items.asMap().entries.map((entry) {
          ...
          return TermsBulletWidget(text: item, isLastItem: isLastItem);
        }),

...
class ProfileController extends ChangeNotifier {
    ...
    if (isLoggedIn) {
      ...
      if (savedAddresses.isNotEmpty) {
        ...
        if (isSignup) {
          ...
          addresses = List.from(AddressModelDummy.defaultAddresses);
          await _saveAddressesToPrefs(addresses, email);
        }
      }
  ...
  Future<void> _saveAddressesToPrefs(
    List<AddressModel> addresses,
    String email,
  ) async {
    final prefs = await SharedPreferences.getInstance();
    ...
    await prefs.setStringList(addressKey, addressList);
    await prefs.setStringList(titlesKey, titleList);
  }

  Future<void> _saveCurrentAddresses() async {
    if (_profile.isLoggedIn) {
      await _saveAddressesToPrefs(_profile.addresses, _profile.email);
    }
  }
  ...
  Future<void> addNewAddress(BuildContext context) async {
    ...
    if (result != null && result is Map) {
      ...
      final updatedAddresses = [..._profile.addresses, newAddress];
  ...
  Future<void> editAddress(BuildContext context, int index) async {
